...
import { createRequirementChat } from '../services/geminiService';
...
export const ChatModal: React.FC<ChatModalProps> = ({ requirement, onClose }) => {
    ...
    >
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-2xl h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}>
        <header className="p-4 border-b flex justify-between items-center">
          <h2 id="chat-modal-title" className="text-lg font-bold text-gray-800">Discussion sur l'exigence N°{requirement.id}</h2>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {messages.map((msg, index) => (
            <div key={index} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-prose p-3 rounded-lg ${msg.role === 'user' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-800'}`}>
